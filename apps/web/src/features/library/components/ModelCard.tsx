...
const ModelCard = ({ 
  ...
  if (variant === 'installed') {
    return (
      <div className="p-4 rounded-2xl bg-white dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-800 flex items-center justify-between group hover:border-zinc-300 dark:hover:border-zinc-700 transition-all">
        ...
        <div className="flex items-center gap-2">
          <div className="px-2 py-1 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 rounded-lg text-[10px] font-bold uppercase tracking-widest flex items-center gap-1">
            <ShieldCheck size={12} />
            {t('common.ready')}
          </div>
          {onDelete && (
  ...
  return (
    <div 
      className={cn(
        "group p-5 rounded-2xl border transition-all duration-300 flex flex-col justify-between",
        isDownloaded 
        ...
        {pullStatus ? (
          <div className="space-y-2">
            <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-zinc-500">
              ...
              {pullStatus.progress !== undefined && <span>{formatNumber(pullStatus.progress)}%</span>}
            </div>
          </div>
        ) : (
          <button
