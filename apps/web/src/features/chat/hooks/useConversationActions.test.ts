...
const mockStopBatching = stopBatching as any;
...
describe('useConversationActions', () => {
  ...
  it('creates a new conversation with current model and settings', () => {
  ...
  it('aborts streaming when deleting conversation', () => {
    const mockStopBatchingFn = vi.fn();
    mockStopBatching.mockImplementation(mockStopBatchingFn);
    mockUseStreamingStore.getState.mockReturnValue({
    ...
    expect(mockStopBatchingFn).toHaveBeenCalledWith('conv1');
  });
  ...
  it('aborts all active streams before clearing', () => {
    ...
    expect(mockStopBatchingFn).toHaveBeenCalled();
