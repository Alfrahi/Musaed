  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-2">
        ...
        <input
          type="range" min="0" max="1" step="0.1" value={globalSettings.temperature}
      ...
      <div className="grid grid-cols-2 gap-4">
        <SliderControl
          label={t('settings.topP')} value={globalSettings.top_p} min={0} max={1} step={0.1}
          displayValue={formatNumber(globalSettings.top_p, { minimumFractionDigits: 1 })}
          onChange={(v) => updateGlobalSettings({ top_p: v })}
        />
        <SliderControl
          label={t('settings.topK')} value={globalSettings.top_k} min={0} max={100} step={1}
      ...
      <div className="grid grid-cols-2 gap-4 pbs-1">
        <NumberInput
          label={t('settings.contextWindow')} value={globalSettings.num_ctx} fallback={2048}
          onChange={(v) => updateGlobalSettings({ num_ctx: v })}
        />
        <NumberInput
          label={t('settings.maxTokens')} value={globalSettings.num_predict} fallback={2048}
