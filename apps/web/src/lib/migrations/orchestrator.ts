...
/**
 * Extracts version from persisted state.
 * Handles both wrapped (with metadata) and unwrapped state.
 */
export function extractVersion(persistedState: unknown): number {
  ...
  // Check for wrapped metadata (new format)
  if (state.metadata && typeof state.metadata === 'object') {
    const meta = state.metadata as Partial<MigrationMetadata>;
    if (typeof meta.version === 'number') {
      return meta.version;
    }
  }

  // Check for flat version (old format / Zustand default)
...
/**
 * Safely extracts data from persisted state.
 * Handles both wrapped and unwrapped formats.
 */
export function extractData<T>(persistedState: unknown): T | undefined {
  if (typeof persistedState !== 'object' || persistedState === null) {
    return undefined;
  }
